...
offsets_t *get_offsets(void)
{
    ...
    {
        if (strncmp(offsets[i]->constant.release, release, strlen(offsets[i]->constant.release)) == 0)
        {
            if (offsets[i]->constant.cpu_subtype == cpu_subtype ||
                offsets[i]->constant.cpu_subtype == CPU_SUBTYPE_ARM_ALL) {
                return offsets[i];
            }
        }
    }
